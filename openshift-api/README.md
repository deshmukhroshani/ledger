...
### Health Check
```bash
curl https://hackathon-api-off-the-ledger.apps.hackathon.francecentral.aroapp.io/
```

### Get a File
```bash
curl https://hackathon-api-off-the-ledger.apps.hackathon.francecentral.aroapp.io/file/TestFile.txt
```

### List Files
```bash
curl https://hackathon-api-off-the-ledger.apps.hackathon.francecentral.aroapp.io/files
```
...
1. **Check pod status**:
   ```bash
   oc get pods -n off-the-ledger-official
   ```

2. **View application logs**:
   ```bash
   oc logs -n off-the-ledger-official deployment/hackathon-api
   ```

3. **Access debug endpoint**:
   ```bash
   ROUTE_URL=$(oc get route hackathon-api -n off-the-ledger-official -o jsonpath='{.spec.host}')
   curl -k https://$ROUTE_URL/debug | jq .
   ```

4. **Run diagnostic test**:
   ```bash
   oc exec -n off-the-ledger-official deployment/hackathon-api -- python /app/test-wif.py
   ```